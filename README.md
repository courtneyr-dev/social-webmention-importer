...
- **Reusable identities** — check "Use this identity for future imports" and your corrected name/profile/avatar apply to every later import once the page confirms the same author (X oEmbed, LinkedIn JSON-LD, or a tested title pattern). Reviewer-confirmed values always outrank parser output. On a blocked or unreadable row — a LinkedIn authwall/999 response, or an X row where both oEmbed and the page fail — nothing is guessed onto it from the saved identity; enter the name yourself.
